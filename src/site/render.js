...
const quoteLink = label => `<a class="button button-gold" href="${esc(business.quotePortalUrl)}" target="_blank" rel="noopener noreferrer">${esc(label)} <span aria-hidden="true">â†’</span></a>`

function serviceCards() {
  return `<div class="card-grid">${services.map(service => `<article class="card"><p class="eyebrow">Starting at ${service.price}</p><h3>${esc(service.name)}</h3><p>${esc(service.summary)}</p><a class="text-link" href="/services/${service.slug}/">Explore this service <span aria-hidden="true">â†’</span></a></article>`).join('')}</div>`
}
