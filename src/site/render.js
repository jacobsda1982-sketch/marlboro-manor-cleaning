...
function quoteExperience() {
  return `<section class="section quote-experience"><div class="container"><div class="quote-intro"><div><p class="eyebrow">Secure estimate request</p><h2>One form. A clear next step.</h2><p>Our secure estimate application verifies the service area and sends your property details directly to Marlboro Manor Cleaning. No payment is collected and no appointment is created at submission.</p></div><div class="quote-benefits"><span>About five minutes</span><span>Draft saves automatically</span><span>Mobile friendly</span></div></div><div class="quote-frame-shell quote-launch-card"><p class="eyebrow">Ready when you are</p><h2>Open the secure estimate form.</h2><p>Google hosts the protected form in a separate secure window. Your website session remains private, and you can return here at any time.</p><a class="button button-gold" href="${esc(business.quotePortalUrl)}" target="_blank" rel="noopener noreferrer" data-conversion="quote_form_launch">Start My Estimate <span aria-hidden="true">&rarr;</span></a><p class="quote-fallback">Prefer email? Contact <a href="mailto:${esc(business.quotesEmail)}">${esc(business.quotesEmail)}</a>.</p></div></div></section>`
}
